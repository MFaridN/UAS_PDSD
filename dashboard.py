...
# testing
def air_quality_regression(data1):
    X = data1[['TEMP']]
    y = data1['PM2.5']
    X = sm.add_constant(X)
    model = sm.OLS(y, X)
...
def perform_clustering(data1):
    X = data1[['TEMP', 'PRES', 'WSPM']]
...
if (menu == "Dashboard"):
    ...
    with tab1 :
      # testing
      # visualisasi_clustering(data_bersih)

      # visualisasi_regresi(data_bersih)
    ...
    visualisasi_clustering(data_bersih_hourly)

    visualisasi_regresi(data_bersih_hourly)
